...
The hard part is that the robot is usually moving, so it also has to factor in its own velocity. It aims at where it needs to be rather than where it currently is. That's what lets it shoot accurately without stopping first. All of it runs off the odometry from the drivetrain and updates every loop.

### Two seasons later

I took this team from a tangled codebase with no real architecture to a robot that plans its own autonomous and aims itself while moving. There were smaller wins along the way, like using a new IMU to auto-balance the robot on the charge-station platform in the 2023 game. That took one weekend and worked almost immediately, unlike everything else here.
