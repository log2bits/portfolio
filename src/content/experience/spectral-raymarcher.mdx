...
But it looks best at two, which is what the demo uses. Here's the part I like. The second sample isn't just a second wavelength, it's offset by the same 256-value dither, so the two samples in a pixel land on a different pair of wavelengths than the two in the pixel next door. Across the screen that's 256x256, about 65,000 distinct wavelength combinations, and each pixel only ever traces two of them. The whites clean right up. It's cheap because the hard part isn't being computed, it's being arranged.
